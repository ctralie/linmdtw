function matchPoints(xpoints, ypoints, isSubSequence){
    ...
    if(!isSubSequence){
        ...
        for(var i = 0; i < xpoints.length - 1; i++){
            subDists[i][0] = distances[i][0];
        }

        //make the sub sequence specific "accumulated distancs" array
        for(var i = 1; i <= xpoints.length -1; i++){
            for(var j = 1; j <= ypoints.length -1; j++){
                
                //the total distance will come from the minimum of the three options added to the distance
                subDists[i][j] = (Math.min(subDists[i-1][j-1], subDists[i-1][j], 
                    subDists[i][j-1], subDists[i][j]) + distances[i][j]);
...
function subSequence(subDists, matches){
    ...
    do{
        ...
        for(var i = 0; i < xpoints.length - 1; i++){
            //if the current distance is less than the next, then it is the minimum
            if(subDists[xpoints.length-1][i] < subDists[xpoints.length-1][i+1]){
                //minimum is found and is stored in a variable
                minimum = [xpoints.length - 1, i];
            }
        }

        i++;
    }while (i < subDists[0].length && minimum == 0);
