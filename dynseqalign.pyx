...
@cython.boundscheck(False)
@cython.wraparound(False)
def DTW_Diag_Step(numpy.ndarray[float,ndim=2,mode="c"] X not None, numpy.ndarray[float,ndim=2,mode="c"] Y not None, numpy.ndarray[float,ndim=1,mode="c"] d0 not None, numpy.ndarray[float,ndim=1,mode="c"] d1 not None, numpy.ndarray[float,ndim=1,mode="c"] d2 not None, int diagLen, int i, int debug, numpy.ndarray[float,ndim=2,mode="c"] U not None, numpy.ndarray[float,ndim=2,mode="c"] L not None, numpy.ndarray[float,ndim=2,mode="c"] UL not None):
    assert(X.shape[1] == Y.shape[1])
    cdef int M = X.shape[0]
    cdef int N = Y.shape[0]
    cdef int dim = X.shape[1]
    dynseqalign.c_diag_step(&X[0,0], &Y[0,0], dim, &d0[0], &d1[0], &d2[0], M, N, diagLen, i, debug, &U[0, 0], &L[0, 0], &UL[0, 0])